...
const commands = [
	new SlashCommandBuilder()
		...
		.setDescription("Create a support message"),
	new SlashCommandBuilder()
		.setName("product")
		.setDescription("Get information about a product")
		.addStringOption(
			(option) =>
				option
					.setName("query")
	...
	new SlashCommandBuilder()
		.setName("order")
		.setDescription("order a product")
...
rest.put(Routes.applicationGuildCommands(discord.clientId, discord.guildId), {
	body: commands,
})
	.then((data) =>
		console.log(
...
client.on("interactionCreate", async (interaction) => {
	...
	switch (commandName) {
		...
		case "product":
			...
			if (!query) {
				axios.get(`${medusa.baseUrl}/store/products`).then((res) => {
					let options = [];
					let i = 0;
					while (i < res.data.products.length) {
						options[i] = {
							label: `${res.data.products[i].title}`,
							description: `${res.data.products[
								i
		...
		case "order":
			...
			if (!interaction.options.getString("id")) {
				...
				let embed = new EmbedBuilder()
					...
					.setDescription(
						`\n\nOrder ID: **${order_id}** - Order Success!\n`,
					)
					.setColor(0x00ae86);
				...
				axios
					.get(
						`${
							medusa.baseUrl
						}/store/orders/${interaction.options.getString("id")}`,
					)
					.then((res) => {
							...
							if (result.length > 0) {
								...
								connection.query(
									...
									(err, result) => {
										if (err) throw err;
										console.log(
											"Added one user to the database",
										);
									},
								);
							}
